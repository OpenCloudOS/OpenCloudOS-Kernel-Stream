	...
	struct iphdr *iph, void *trh)
{
	...
	*/
	...
	sip = find_sockinfo(IPPROTO_TCP, &key, sizeof key.tcp4, bs);
	if (sip == NULL) {
		// no sockinfo yet: create one
		sip = make_sockinfo(IPPROTO_TCP, &key, sizeof key.tcp4, bs);
		if (sip == NULL) {
			if (direction == 'i')
				unidenttcprcvpacks++;
			else
	...
	struct iphdr *iph, void *trh)
{
	...
	*/
	...
	sip = find_sockinfo(IPPROTO_UDP, &key, sizeof key.udp, bs);
	if (sip == NULL) {
		// no sockinfo yet: create one
		sip = make_sockinfo(IPPROTO_UDP, &key, sizeof key.udp, bs);
		if (sip == NULL) {
			if (direction == 'i')
				unidentudprcvpacks++;
			else
	...
	int in_syscall, char direction)
{
	...
	if (*tipp == NULL) {
		...
		*tipp = get_taskinfo(curid, idtype);
		if (*tipp == NULL) {
			/*
...
make_sockinfo(int proto, union keydef *identp, int identsz, int hash)
{
	...
	*/
	...
	sip = kmem_cache_alloc(sicache, GFP_ATOMIC);
	if (sip == NULL)
		return NULL;
...
get_taskinfo(pid_t id, char type)
{
	...
	*/
	tip = kmem_cache_alloc(ticache, GFP_ATOMIC);
	if (tip == NULL)
		return NULL;
	...
	*/
	...
	if (current->start_boottime.tv_nsec + boottime.tv_nsec > NSEC_PER_SEC)
		tip->btime++;
#endif
