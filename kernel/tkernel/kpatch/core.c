...
static struct stack_trace_kpatch trace = {
...
static int kpatch_verify_activeness_safety(struct kpatch_module *kpmod)
{
	...
	do_each_thread(g, t) {
		...
		trace.nr_entries = 0;
		save_stack_trace_tsk_kpatch(t, &trace);
				     ...
				     unsigned long sympos, unsigned long *addr)
{
	...
	if (vmlinux) {
		...
		module_kallsyms_on_each_symbol(kpatch_kallsyms_callback, &args);
	}
...
"that the binary patch does not handle paravirt operations, alternatives or the like properly.\n");
     ...
     (LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0) && \
      UTS_UBUNTU_RELEASE_ABI >= 7) \
		...
		ret = copy_to_kernel_nofault((void *)loc, &val, size);
